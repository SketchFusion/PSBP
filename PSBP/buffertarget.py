...
if __name__ == "__main__":
        ...
        buffer_series = []
        time_series1 = []
        with open("./bandwidth_fcc.txt", "r") as f:
            while True:
                try:
        ...
        with open("./buffer_fcc.txt", "r") as f1:
            while True:
                try:
                    t, c = map(float, (f1.readline().split())[:2])
                    if time_series1 and abs(c - buffer_series[-1]) < 1e-3:
                        pass
                    else:
                        time_series1.append(t)
                        buffer_series.append(c)
                except:
                    break
        print(len(buffer_series))
        ...
        thres = 2 * std
        hazard = np.sum(
            np.sum((bandwidth_series[start:end] > mean + thres) | (bandwidth_series[start:end] > mean + thres))) / T
        print(T, hazard)
        ...
        for i in range(1, len(pre.Gt_list)):
            if (pre.Gt_list[i] - pre.Gt_list[i - 1]) < 0:
                ...
                if len(bandwidth_series[last:i]) >= m0:
                    ...
                    while s + m0 <= i - 1:
                        idx_start = s + m0
                        if idx_start >= len(bandwidth_series):
                            break
                        print(s,"s","m0",m0)
                        # 获取当前 buffer duration
                        print("idx_start",idx_start)
                        buffer_duration = buffer_series[idx_start]
                        print("buffer_duration",buffer_duration)
                        t_start = time_series[idx_start]
                        print("t_start",t_start)
                        acc_time = 0
                        acc_bandwidth = []
                        idx_end = idx_start

                        # 向后找时间段覆盖 buffer duration
                        while idx_end + 1 < len(time_series) and acc_time < buffer_duration:
                            acc_time = time_series[idx_end + 1] - t_start
                            acc_bandwidth.append(bandwidth_series[idx_end])
                            idx_end += 1
                        # 如果找到一段覆盖了 buffer duration 的片段
                        if acc_time >= buffer_duration and len(acc_bandwidth) > 0:
                            avg = np.mean(acc_bandwidth)
                            X.append(bandwidth_series[s:s + m0])
                            print(bandwidth_series[s:s + m0])
                            y.append(bandwidth_series[idx_start])
                            print(bandwidth_series[idx_start])
                            y_avg.append(avg)
                            buffer_seg.append(buffer_series[s:s + m0])
                        s += 1
                    last = i
                '''
                    while s + m0 <= i - 1:
        ...
        #np.savez('./bandwidth_seg.npz', X=X_array, y=y_array, y_avg=y_avg_array)
        buffer_seg_array = np.array(buffer_seg)
        np.savez('./bandwidth_frame_seg_fcc.npz', X=X_array, y=y_array, y_avg=y_avg_array)
        np.savez('./buffer_frame_seg_fcc.npz',  buffer=buffer_seg_array)